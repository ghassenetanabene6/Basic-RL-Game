...
alphaD = 1 #decay factor of the learning rate
gamma = 0.9 #discount factor

if mode == "play":
    e = 0.0001 #randomness
    ed = 1 #decay factor of e
    emin = 0.0001
else:
    e = 0.9
    ed = 1.3
...
try:
    with open("saved\Q-model.pickle", "rb") as file:
...
def onGameOver(score, moves):
    ...
    if gameCounter % 200 == 0:
        with open("saved/" + "Q-model" + ".pickle","wb" ) as file:
...
if mode == "play":
    snake1.main(emulate, onGameOver, onScore)
else:
    snake2.main(emulate, onGameOver, onScore)
